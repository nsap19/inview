<template>
  <div :class="this.$route.path === '/home' ? 'wrapper-home' : 'wrapper'">
    <div
      ...
      v-if="
        this.$route.path !== '/' && !this.$route.path.startsWith('/meeting')
      "
    >
      <div
        :class="[
          ...
          this.$route.path === '/home' ? 'nav-home' : 'nav',
        ]"
      >
        <!-- 로고 -->
        <div
        ...
        <div class="d-flex flex-row justify-content-end col-6 col-sm-3 p-0">
          ...
          <div v-if="Object.keys(user).length" class="d-flex flex-row">
            <el-button
              ...
              round
              plain
              @click="openCreateMeetingDialog = true"
              size="large"
            ></el-button>
            <el-dropdown>
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item @click="UpdateUserModal"
                    >회원 정보 수정</el-dropdown-item
                  >
                  <el-dropdown-item @click="this.$store.dispatch('logout')"
          ...
          <Login
            ...
            v-on:password="
              [(openFindPasswordDialog = true), (openLoginDialog = false)]
            "
            v-on:signup="[(openSignupDialog = true), (openLoginDialog = false)]"
          />
          <Register
            v-model="openSignupDialog"
            v-on:login="[(openSignupDialog = false), (openLoginDialog = true)]"
          />
          <UpdateUser
            v-model="openUserUpdateDialog"
            v-on:updateUser="[(openUserUpdateDialog = false)]"
          />
          <FindPassword
            v-model="openFindPasswordDialog"
            v-on:password="[(openFindPasswordDialog = false)]"
          />
        </div>
      </div>
    </div>
